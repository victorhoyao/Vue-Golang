...
// 获取订单列表
func GetOrderListYK3(page int, list_rows int, status int) map[string]interface{} {
	resMap := make(map[string]interface{})

	ip := viper.GetString("平台ip.易客3")
	url := fmt.Sprintf("/api/supplier/order/v2/orders?page=%d&goodsSN=&orderSN=&state=%d&limit=%d", page, status, list_rows)

	AppID := viper.GetString("Ukey3.AppID")
	AppSecret := viper.GetString("Ukey3.AppSecret")
