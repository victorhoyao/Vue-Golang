...
var YK3typeList []int = []int{4179, 4178} //易客3type

// 易客3拉单
func GetHigherOrdersYK3(db *gorm.DB) {
	...
	defer func() {
		if err := recover(); err != nil {
			fmt.Println(fmt.Sprintf("易客3拉取订单recover%s", Tools.GetDateNowFormat(true)))
			go GetHigherOrdersYK3(db)
		}
	}()
	...
	for {
		fmt.Println(fmt.Sprintf("易客3拉取订单%s,page%d", Tools.GetDateNowFormat(true), LDpage))
		resMap := BLTaskFunc.GetOrderListYK3(LDpage, 50, 2)
		if resMap["ret"].(bool) == false {
			fmt.Println("易客3拉取订单失败，" + resMap["msg"].(string))
		...
		if len(infos) == 0 {
			fmt.Println(fmt.Sprintf("易客3拉取订单完成len=0 %s", Tools.GetDateNowFormat(true)))
		...
		for _, value := range infos {
			...
			if isCodeIn(YK3typeList, goods_id) == false {
				continue
			}
		...
		fmt.Println(fmt.Sprintf("易客3插入新订单 %d 条 %s", len(newRecords), Tools.GetDateNowFormat(true)))

		for _, record := range newRecords {
			fmt.Println(fmt.Sprintf("易客3 拉单 %d", record.OrderId))
		}
...
// 易客3退单
func GetTDOrdersYK3(db *gorm.DB, status int) {
	time.Sleep(time.Second * time.Duration(7))
	timer := viper.GetInt("Timer.UpdateTDOrderTimer")

	defer func() {
		if err := recover(); err != nil {
			fmt.Println(fmt.Sprintf("易客3退单recover%s", Tools.GetDateNowFormat(true)))
			go GetTDOrdersYK3(db, status)
		}
	}()
	...
	for {
		tx := db.Begin()
		resMap := BLTaskFunc.GetOrderListYK3(TDpage, 50, status)
		fmt.Println(fmt.Sprintf("易客3退单%d订单%s,page%d", status, Tools.GetDateNowFormat(true), TDpage))
		if resMap["ret"].(bool) == false {
			fmt.Println(fmt.Sprintf("易客3退单%d订单失败,%s", status, resMap["msg"].(string)))
			time.Sleep(time.Second)
			continue
		}
		data := resMap["data"].(map[string]interface{})
		...
		fmt.Println(fmt.Sprintf("易客3退单%d订单%s完成", status, Tools.GetDateNowFormat(true)))
		time.Sleep(time.Second * time.Duration(timer))
	}
}
